...
parser = ArgumentParser(description='ISTA-Net for Image Deblurring - FIXED VERSION')
...
# ============================================================================
# MODEL: ISTA-Net for Deblurring - FIXED
# ============================================================================

class ISTABlock(nn.Module):
    """Single ISTA layer for image deblurring - FIXED WITH BETTER INITIALIZATION."""
    def __init__(self):
        super(ISTABlock, self).__init__()
        
        # FIXED: Better initialization for proper gradient flow
        # eta_raw = 0 gives softplus(0) = log(2) ≈ 0.693 (good step size)
        # lam_raw = -1 gives softplus(-1) ≈ 0.313 (good threshold)
        # These allow gradients to flow properly through the network
...
print(f"Initialized ISTA-Net with {layer_num} layers (FIXED)")
...
model_dir = "./%s/ISTA_Net_Deblur_layer_%d_lr_%.4f_FIXED" % (args.model_dir, layer_num, learning_rate)
...
for epoch in range(1, end_epoch + 1):
    ...
    print(f"[Epoch {epoch}/{end_epoch}] Average Loss: {avg_loss:.6f}")
